...
class SATInstance:
    ...
    def evaluate(self, partial_truth_assignment):
        ...
        for c in self.clauses:
            clause_status = [self.evaluate_literal(partial_truth_assignment, l) for l in c]
            if all(c == -1 for c in clause_status):
                # return clause violated
                return -1
            if any(c == 1 for c in clause_status):
                continue
...
#                 # if x or y or z = 1 then true... want to pass those returns...
#                 #pass
#                 continue
#             elif sum(clause_evaluation) == -3:
#                 # all three are determinate false
#                 return -1
#             else: 
#                 # Every element of the current clause is indeterminate
#                 return 0
            
#         #made it to the end without a return, return 1
#         return 1
...
#     #--- First Attempt
#     try1 = extend_truth_assignment(partial_truth_assign, j, True)
#     #print("Try1 = ",try1)
#     try1e = formula.evaluate(try1)  
#     if try1e == 1:
#         #print(f"success, end early on {j} True")
#         return [True,try1]
#     if try1e ==0:
#         if j<= formula.n -1:
#             (result,lastassign)=dpll_algorithm(formula,try1,j+1)
#             if result == True:
#                 return [True,lastassign]    
#     truth_assign = {}
#     try2 = extend_truth_assignment(partial_truth_assign, j, False)
#     #print("Try2: ",try2)
#     try2e = formula.evaluate(try2)
#     if try2e == 1:
#         #print(f"success, end early on {j} False")
#         return [True,try2]
#     if try2e == 0:
#         if j<= formula.n -1:
#             (result,lastassign)=dpll_algorithm(formula,try1,j+1)
...
#   return FAILURE, None
    ...
    if try2e == 0:
        if j<= formula.n -1:
            (result,lastassign)=dpll_algorithm(formula,try2,j+1)
            if result == True:
                return (True,lastassign) 
...
def is_three_coloring(graph, coloring):
    ...
    # your code here
    for i in range(len(graph.get_list_of_edges())):
        a,b = graph.get_list_of_edges()[i]
        if coloring[a] == [b]:
            return False
    #print(graph.n)
    #print(coloring)
...
# vertex  𝑖  must be definitively shaded from set {1,2,3}
# $$ X_{𝑖,1} X_{𝑖,2} X_{𝑖,3} $$.
#  
# vertex  𝑖 cannot have more than one color.
#     
#     𝗇𝗈𝗍 X𝑖1 𝗈𝗋 𝗇𝗈𝗍 X𝑖2
#     𝗇𝗈𝗍 X𝑖2 𝗈𝗋 𝗇𝗈𝗍 X𝑖3
#     𝗇𝗈𝗍 X𝑖1 𝗈𝗋 𝗇𝗈𝗍 X𝑖3
#  
#  
# for each edge  (𝑖,𝑗)  vertices  V𝑖,V𝑗  cannot both have the same color.
#     
#     𝗇𝗈𝗍 X𝑖1 𝗈𝗋 𝗇𝗈𝗍 X𝑗1
#     𝗇𝗈𝗍 X𝑖2 𝗈𝗋 𝗇𝗈𝗍 X𝑗2
#     𝗇𝗈𝗍 X𝑖3 𝗈𝗋 𝗇𝗈𝗍 X𝑗3
...
#     for y in graph.get_list_of_edges():
#         i = y[0]
#         j = y[1]
#         s.add_clause([3*i + 1, 3*i + 2, 3*i + 3]) #Neither i nor j can have any color but 1 2 or 3
#         s.add_clause([3*j + 1, 3*j + 2, 3*j + 3])
# #         s.add_clause([-(3*i + 1), -(3*i + 2)])  # Both i can only be one color
# #         s.add_clause([-(3*i + 1), -(3*i + 3)])
# #         s.add_clause([-(3*i + 2), -(3*i + 3)])
# #         s.add_clause([-(3*j + 1), -(3*j + 2)])  # Cont'd j can only be one color
# #         s.add_clause([-(3*j + 1), -(3*j + 3)])
# #         s.add_clause([-(3*j + 2), -(3*j + 3)])
        
#         s.add_clause([-(3*i + 1), -(3*i + 2), (3*i + 3)])  # Both i can only be one color
#         s.add_clause([-(3*i + 1), -(3*i + 3), (3*i + 2)])
#         s.add_clause([-(3*i + 2), -(3*i + 3), (3*i + 1)])
#         s.add_clause([-(3*j + 1), -(3*j + 2), (3*j + 3)])  # Cont'd j can only be one color
#         s.add_clause([-(3*j + 1), -(3*j + 3), (3*j + 2)])
#         s.add_clause([-(3*j + 2), -(3*j + 3), (3*j + 1)])
...
#         s.add_clause([-(3*i + 3), -(3*j + 3)])
    ...
    for i in range(graph.n):
            ...
            s.add_clause([-(3*i+2), -(3*i+1),(3*i+3)])
            s.add_clause([-(3*i+2), -(3*i+3),(3*i+1)])
            s.add_clause([-(3*i+1), -(3*i+3),(3*i+2)])
    for (i,j) in graph.get_list_of_edges():
        ...
        xj2 = 3*j + 2
        xj3 = 3*j + 3
        s.add_clause([-xi1, -xj1])
        s.add_clause([-xi2, -xj2])
        s.add_clause([-xi3, -xj3])
...
#             colors[current_vertex] = current_color
    for i in range(graph.n):
        k = i
        if truth_assign[k+1]==True:
            colors[k]=1
        elif truth_assign[k+2] == True:
            colors[k]=2
        else:
            colors[k]=3
    #print(colors)
...
def solve_three_coloring(graph):
    ...
    #print("res",res)
    #print("truth_assign",truth_assign)
    if res: 
        return extract_graph_coloring_from_truth_assignment(graph, truth_assign)
    else: 
        return None

##Below is Test Timing for The Cell Below.
## Case 5 and 6 are taking > 90% of the time for the whole notebook...
...
en=time.time()
print(F"Total = {en-st}")
print(F'Total in evaluate: {tot}')
print(F'Percent of total {tot}/{en-st} = {100*tot/(en-st)}% ')
